...
int fsm_write_file(const char* path, char* buf, unsigned long size) {
        ...
        /* LAB 5 TODO BEGIN */
        struct ipc_msg *msg_cap = ipc_create_msg(fsm_ipc_struct, sizeof(struct fs_request), 0);
        struct fs_request *fr_cap = (struct fs_request *) ipc_get_msg_data(msg_cap);

        fr_cap->req = FS_REQ_GET_FS_CAP;
        strcpy(fr_cap->getfscap.pathname, path);
        ipc_call(fsm_ipc_struct, msg_cap);

        int cap = ipc_get_msg_cap(msg_cap, 0);
        struct fs_cap_info_node * fs_cap_info = get_fs_cap_info(cap);
        struct ipc_msg *msg = ipc_create_msg(fs_cap_info->fs_ipc_struct, sizeof(struct fs_request) + size + 1, 0);
        ...
        fr->open.new_fd = fd;
        strcpy(fr->open.pathname, fr_cap->getfscap.pathname);
        ipc_call(fs_cap_info->fs_ipc_struct, msg);
        ...
        fr->req = FS_REQ_CLOSE;
        fr->close.fd = fd;
        ret = ipc_call(fs_cap_info->fs_ipc_struct, msg);
        
        ipc_destroy_msg(fsm_ipc_struct, msg_cap);
        ipc_destroy_msg(fs_cap_info->fs_ipc_struct, msg);
        /* LAB 5 TODO END */
...
int fsm_read_file(const char* path, char* buf, unsigned long size) {
        ...
        memcpy(buf, ipc_get_msg_data(msg), ret);
        fr->req = FS_REQ_CLOSE;
        fr->close.fd = fd;
        ret = ipc_call(fs_cap_info->fs_ipc_struct, msg);
